...
public class EZIDARKService {
    ...
    public EZIDARKService(String uri, String naan, String username, String password) {
        this.uri = uri;
        this.naan = naan;
    ...
    public String mint(String shoulder) throws HTTPException {
    	
    	String noid = mint(shoulder, null);
    ...
    /**
     * Mint a new ARK and bind it to the given URL
     * 
     * @returns The NOID identifier portion of the ARK
     */
    
    public String mint(String shoulder, String target_url) throws HTTPException {
    	
    	// If the minting resource has not already been instantiated, then make a static resource that
    	//  can be reused
        if (mintResource == null) {
        	String url = this.uri;
            mintResource = rootResource.path("shoulder").path("ark:/"+naan+"/"+shoulder);
        }
